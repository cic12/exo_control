...
struct testParams {
	bool device = 0, sim = 1;
	int human = 0, analogIn = 0, n_ai = 4;
	int control = 1; // None, MPC, PID, Imp
	int config = 0;
	int traj = 0;
	double T = 4.0;
...
struct mpcParams {
	...
	const double x0[NX] = { 0.25 , 0.1 , 0.0 , 1.0 };
...
struct pidImpParams {
	int type = 0;
	double Kp = 10, Ki = 0, Kd = 0, Kff_A = 0, Kff_B = 0, Kff_J = 0, Kff_tau_g = 0, alpha_err = 0.05, lim = 20;
};
...
public:
	...
	void emgSimProcess(char emg_string[]);
	void control_loop();
	void control_stop();
	void daqProcess();
