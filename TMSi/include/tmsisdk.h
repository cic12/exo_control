/*
Copyright (c) 2012-2014 TMSi B.V.
All rights reserved.
...
// for MEASURE_MODE_CALIBRATION: (obsolete)
...
#define UNIT_GRAVITY 9		// Used for accelerometer
...
typedef struct _FRONTENDINFO
{
    ...
    unsigned short maxRS232;	/*!<  Max. sample frequency over serial connection */
    ...
    unsigned short RecBufSize;	/*!<  Do not use, for debugging only */
    unsigned short SendBufSize;	/*!<  Do not use, for debugging only */
    unsigned short NrOfSwChannels;   /*!<  Max. number of channels supported by this device */
    unsigned short BaseSf;		/*!<  Max. sample frequency */
    unsigned short Power;		/*!<  Do not use */
    unsigned short Check;		/*!<  Do not use */
...
#define CHANNELTYPE_ACCEL 12

/* Obsolete define for feature mode. Do not use them! */
...
// Use these two defines as default value for SetSignalBuffer
...
typedef struct TMSiTDFHeader		// Detailed description can be found in the TDF spec document.
{
    ...
    unsigned short		RecordingSampleRate;		/*!< Recording samplerate used for this recording */
...
typedef enum _TMSiStartControl {
    sc_man_shutdown_enable = 256, /* !< If this bit is set, the user can manually shutdown the system. Otherwise, the system can only shut down automatically */
    sc_rf_recurring = 128, /* !< If this bit is set, the system will reinitialize the RF ALARM after the previous ALARM, based on the Wireless Time values specified */
    sc_rf_timed_start = 64, /* !< If this bit is set, the system will enable the RF module according to the Wireless interface parameters. */
    sc_rf_auto_start = 32, /* !< If this bit is set, the system will enable the RF module directly after power-up. */
    sc_alarm_recurring = 16, /* !< If this bit is set, the system will reinitialize the RTC ALARM after the previous ALARM, based on the AlarmTime values specified. */
    sc_power_on_record_auto_start = 8, /* !< If this bit is set, recording is started automatically at power on, without the need for user intervention. If this bit is not set, data will not be stored on the SD card. */
    sc_man_record_enable = 4, /* !< If this bit is set then the recording can be started and stopped manually, by starting the device over WiFi! This bit should be used in combination with sc_rf_auto_start */
    sc_alarm_record_auto_start = 2, /* !< If this bit is set the alarm settings of the real-time clock are used to automatically start the recording. The auto start time is programmed at the moment that the user switches the TMSi system on for the first time, after the bit has been set.. At the same time the bit will be cleared, so that the same auto start time will be not be set again. After the auto start time, this time is also cleared from TMSi system memory. */
    sc_rtc_set = 1 /* !< If this bit (Least significant bit) is set then the ‘Alarm time’ settings  are used to set the RealTime Clock. This happens at the moment that the user switches the TMSi system ON for the first time, after this bit has been set. At the same time the bit will be cleared, so that the same RTC time will be not be set again. */
...
typedef struct TMSiRecordingConfig	// Detailed description can be found in the TDF spec document.
{
    ...
    unsigned short		RecordingSampleRate;/*!< The samplerate of which the recording is made / had to be made */
    ...
    unsigned int		StartControl; /*!< Set by user. This integer consists of a number of bits that control the startup behaviour of the system. See TMSiStartControlType for used bits */
    unsigned int		EndControl;/*!< Set by user. This integer consists of a number of bits that control the startup behaviour of the system. See TMSiStartControlType for used bits */
    ...
    SYSTEMTIME		RFInterfStartTime;						/*!< Not used */
    SYSTEMTIME		RFInterfStopTime;						/*!< Not used */
    SYSTEMTIME		RFInterfInterval;						/*!< Not used */
    unsigned int		RFInterfCount;						/*!< Not used */
...
typedef struct TMSiBatReport {
    ...
    short AccumCurrent; 		/*!<  Battery Accumulated Current in milliAmpereHour (mAh) */
    short AvailableCapacityInPercent; /*!<  Do not use, reserved for future use */
...
typedef struct TMSiDeviceReport
{
    ...
    unsigned int	AdapterCycles;	/*!<  Not used: Number of connections made by the Adapter. */
    unsigned int	MobitaSN;		/*!<  Serial number of the Mobita */
    unsigned int	MobitaStatus;	/*!<  Status of the Mobita : 0=Unknown; 1=Ok;2=MemError;3=BatError; */
    unsigned int	MobitaCycles;	/*!<  Not used: Number of adapter connections made by the Mobita */
    unsigned short	DoNotUse1;		/*!<  Do not use, reserved for future use */
    unsigned short	DoNotUse2;		/*!<  Do not use, reserved for future use */
    unsigned short	DoNotUse3;		/*!<  Do not use, reserved for future use */
    unsigned short	DoNotUse4;		/*!<  Do not use, reserved for future use */
...
typedef struct TMSiExtFrontendInfo
{
    ...
    unsigned short	CurrentBlockType; 	/*!<  TMSi internal use: The blocktype used to send sample data for the selected CurrentFs and selected CurrentInterface */
    unsigned short	DoNotUse1;			/*!<  Do not use, reserved for future use */
    unsigned short	DoNotUse2;			/*!<  Do not use, reserved for future use */
    unsigned short	DoNotUse3;			/*!<  Do not use, reserved for future use */
    unsigned short	DoNotUse4;			/*!<  Do not use, reserved for future use */
...
// NexTus specific
BOOLEAN APIENTRY GetDigSensorData(void *Handle, unsigned int *DataSize, unsigned char *SensorData);
BOOLEAN APIENTRY SetDigSensorData(void *Handle, unsigned int GivenDataSizeInBytes, unsigned char *GivenSensorData);
BOOLEAN APIENTRY GetFlashStatus(IN HANDLE Handle, IN OUT long *Status);
BOOLEAN APIENTRY StartFlashData(void *Handle, unsigned int StartAdress, unsigned int Length);
LONG APIENTRY GetFlashSamples(IN HANDLE Handle, OUT PULONG SampleBuffer, IN ULONG SampleBufferSizeInBytes);
BOOLEAN APIENTRY StopFlashData(void *Handle);
BOOLEAN APIENTRY FlashEraseMemory(IN HANDLE Handle);
BOOLEAN APIENTRY SetFlashData(IN HANDLE Handle, unsigned int StartAdressInBytes, PULONG FlashData, int FlashBlockLengthInWords);
BOOLEAN APIENTRY SetChannelReferenceSwitch(void *Handle, unsigned int GivenDataSizeInBytes, unsigned char *GivenChannelSwitchData);
BOOLEAN APIENTRY TMSIGetRawIdData(IN HANDLE Handle, int KeyCode, unsigned short *NrOfShortsOut, short* OutBuffer);
...
typedef BOOLEAN(__stdcall * PGETDIGSENSORDATA)(void *Handle, unsigned int *DataSize, unsigned char *SensorData);
typedef BOOLEAN(__stdcall * PSETDIGSENSORDATA)(void *Handle, unsigned int GivenDataSizeInBytes, unsigned char *GivenSensorData);
typedef BOOLEAN(__stdcall * PGETFLASHSTATUS)(IN HANDLE Handle, IN OUT long *Status);
typedef BOOLEAN(__stdcall * PSTARTFLASHDATA)(void *Handle, unsigned int StartAdress, unsigned int Length);
typedef LONG(__stdcall * PGETFLASHSAMPLES)(IN HANDLE Handle, OUT PULONG SampleBuffer, IN ULONG SampleBufferSizeInBytes);
typedef BOOLEAN(__stdcall * PSTOPFLASHDATA)(void *Handle);
typedef BOOLEAN(__stdcall * PFLASHERASEMEMORY)(IN HANDLE Handle);
typedef BOOLEAN(__stdcall * PSETFLASHDATA)(IN HANDLE Handle, unsigned int StartAdressInBytes, PULONG FlashData, int FlashBlockLengthInWords);
typedef BOOLEAN(__stdcall * PSETCHANNELREFERENCESWITCH)(void *Handle, unsigned int GivenDataSizeInBytes, unsigned char *GivenChannelSwitchData);
typedef BOOLEAN(__stdcall * PTMSIGETRAWIDDATA)(IN HANDLE Handle, int KeyCode, unsigned short *NrOfShortsOut, short* OutBuffer);
